...
export default function AccountScreen() {
  ...
  async function checkUser() {
    try {
      ...
      if (currentUser) {
        // Get user role from your user_profiles table
     const { data: profile, error } = await supabase
  .from('users')  // <-- Your actual table
  .select('role, username')
  .eq('auth_user_id', currentUser.id)  // <-- Correct field name
  .single();
              ...
              {pendingBars.length === 0 ? (
                ...
                pendingBars.map((bar) => (
                  <View key={bar.id} style={styles.pendingItem}>
                    ...
                    <Text style={styles.itemDetails}>{bar.address}</Text>
                    <Text style={styles.submittedBy}>
